...
// دالة إنشاء تقويم مبسط
function createSimpleCalendar(employeeId: number): InlineKeyboard {
  return Calendar.create({
    callbackPrefix: `hr:employee:status:simple:date:${employeeId}`,
  })
}
  ...
  try {
    ...
    if (isCurrentEmployee) {
      ...
      const keyboard = new InlineKeyboard()
        ...
        .text('🏖️ في إجازة', `hr:employee:status:change:${employeeId}:ON_LEAVE`)
        .row()
        .text('✈️ في مأمورية', `hr:employee:status:change:${employeeId}:ON_MISSION`)
        .row()
        .text('📂 تحويل لعامل سابق', `hr:employee:status:change:${employeeId}:FORMER`)
        .row()
      ...
      const keyboard = new InlineKeyboard()
        ...
        .text('📋 تصفية الحساب', `hr:employee:status:change:${employeeId}:SETTLED`)
        .row()
        .text('⏸️ تعليق الحساب', `hr:employee:status:change:${employeeId}:SUSPENDED`)
        .row()
        .text('⬅️ رجوع', `hr:employee:edit:${employeeId}`)
    ...
    switch (newStatus) {
      ...
      case 'FORMER':
        ...
        const keyboard = new InlineKeyboard()
          ...
          .text('🚫 فصل', `hr:employee:status:former:${employeeId}:TERMINATED`)
          .row()
          .text('👴 تقاعد', `hr:employee:status:former:${employeeId}:RETIRED`)
          .row()
...
employeeStatusSimpleHandler.callbackQuery(/^hr:employee:status:former:(\d+):(.+)$/, async (ctx) => {
  ...
  const keyboard = new InlineKeyboard()
    ...
    .text('📆 تاريخ آخر', `hr:employee:status:simple:date:${employeeId}:custom`)
    .row()
    .text('⬅️ رجوع', `hr:employee:status:simple:${employeeId}`)
...
employeeStatusSimpleHandler.callbackQuery(/^hr:employee:status:simple:date:(\d+):(.+)$/, async (ctx) => {
  ...
  try {
    ...
    if (dateType === 'today') {
      ...
      selectedDate = Calendar.parseDate(dateType)
...
employeeStatusSimpleHandler.on('message:text', async (ctx, next) => {
