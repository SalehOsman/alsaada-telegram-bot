...
export class MenuBuilder {
  ...
  private static buildKeyboardFromSubFeatures(
    ...
    subFeatures.forEach((subFeature, index) => {
      ...
      // If maxPerRow is 1, add button and immediately end the row
      // This ensures each button is on its own row
      if (maxPerRow === 1) {
        keyboard.text(text, callbackData)
        keyboard.row()
      }
        ...
        keyboard.text(text, callbackData)
        // End the row after every maxPerRow buttons
    ...
    // Add final row only if maxPerRow > 1 and there are remaining buttons
