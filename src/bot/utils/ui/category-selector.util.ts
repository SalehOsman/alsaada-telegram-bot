 ...
 * @example
 * ```typescript
 * const categories = await prisma.iNV_Category.findMany();
 ...
 * @example
 * ```typescript
 * const category = await prisma.iNV_Category.findUnique({ where: { id: 1 } });
 ...
 * // 5
 * 
 * const category = await prisma.iNV_Category.findUnique({
 *   where: { id: categoryId }
 * });
 * ```
 */
